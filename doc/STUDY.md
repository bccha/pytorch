...
### 😲 9.5. [심화] CNN은 GPU에서 컴퓨터 친화적으로 어떻게 계산될까? (`im2col`)
우리는 CNN이 돋보기(3x3 필터)를 한 칸씩 이동하며 픽셀을 곱하고 더하는 것(Sliding Window)이라고 상상합니다. 하지만 이를 곧이곧대로 C++이나 CUDA 코드로 구현하면 **4중 for문**이라는 끔찍하게 느린 연산 괴물이 탄생합니다.
* **불편한 진실**: GPU나 NPU 구조는 메모리를 점프하며 조금씩 곱하는 것을 제일 혐오합니다. 대신, **크고 무식한 2차원 행렬 곱셈(GEMM)**을 한 방에 처리하는 데 1만 배 이상 특화되어 있습니다.

#### 🪄 마법의 변환: `im2col` (Image to Column) 구조
천재적인 하드웨어 설계자들은 거대한 돋보기 연산을 행렬 연산 1방으로 둔갑시켰습니다. (`src/09_2_im2col_custom_cnn.py` 참조)

```text
[ 💡 im2col 동작 원리 시각화 (예시: 3x3 사진에 2x2 돋보기를 댈 때) ]

① 공간 연산 (Sliding Window: 4중 for문)
  1  2  3      [돋보기]       [1구역]   [2구역]   [3구역]   [4구역]
  4  5  6   *   a  b     =>   1  2      2  3      4  5      5  6
  7  8  9       c  d          4  5      5  6      7  8      8  9

② im2col 변환 (1차원 막대로 좍좍 펴기) -> 🌟 거대 2D 행렬 곱셈 단 1방으로 끝!🌟
...
1. 💡 **변환(Unfold)**: 돋보기가 쳐다볼 부분을 그대로 복사해서 **기다란 1차원 막대기(Column)**로 쫙쫙 폅니다. 다음 칸 구역도 뽑아서 그 옆 막대기로 스윽 세웁니다.
2. 🧮 **행렬 곱셈(MatMul)**: 필터 가중치 행렬과 방금 쫙 펴둔 거대한 이미지 판때기를 `선형 대수 곱셈 (Linear 레이어와 동일)` 단 1번으로 곱해버립니다!
3. 📦 **결과 접기**: 계산된 1차원 막대기 결과물들을 다시 원래의 예쁜 2차원 이미지(Feature Map) 형태로 스윽 접어놓습니다.

#### 📊 성능 증명 (`09_3_compare_conv_vs_im2col.py`)
이 막대기 복사 과정(`im2col`)으로 인해 메모리(RAM)는 중복 데이터로 엄청나게 낭비(Trade-off)되지만, **수학적인 곱셈 결과값은 순정 `nn.Conv2d`와 오차율 `1e-06` 미만으로 100% 완벽히 똑같습니다.**
즉, 최첨단 GPU의 cuDNN이나 NPU 가속기의 최하단에서는 우리가 짠 CNN 코드를 모두 이 `im2col + 행렬곱(GEMM)` 아키텍처로 몰래 변환하여 수백 배의 '물리적 연산 가속'을 끌어내고 있었던 것입니다!
...
### 💡 (참고) `model.eval()` 로 내린 스위치를 나중에 어떻게 다시 학습 모드(`model.train()`)로 돌려놓나요?
딥러닝 코드에서는 보통 **매 에폭(Epoch)이 시작되는 반복문(`for epoch...`)의 가장 첫 줄**에 `model.train()`을 적어두는 것이 관행입니다.
만약 학습 도중 특정 에폭마다 평가 코드를 섞어쓰더라도, 맨 밑에서 `model.eval()`을 호출해 평가가 먼저 끝나면 코드는 **다시 위쪽 반목문 처음으로 올라가 다음 에폭을 준비**하게 됩니다. 이때 가장 먼저 마주치는 코드가 `model.train()`이므로, 알아서 다시 학습 모드 스위치가 켜지는 아주 안전한 구조로 동작합니다.
...
### 😲 (번외) 머리를 바꾸기 전, 순정 거인은 개미/벌을 맞출 수 있을까?
놀랍게도 아무런 튜닝 없이 **1,000가지 영단어를 외치는 순정 ResNet18 모델**에게 우리 개미/벌 데이터셋을 주면, 무려 **74.5% 확률로 정확히 "ant" 나 "bee"를 외칩니다**. (`10_2_test_original_resnet.py` 참조)
전이 학습이란 거인이 이미 갖고 있는 이 거대한 통찰력을 내 맘대로(단 2개의 클래스로) 예쁘게 정리(100%에 가깝게)하는 최적화 작업일 뿐입니다.

## 16. 순차 데이터의 마술사: RNN (Recurrent Neural Network)
지금까지 배운 CNN이 "사진의 공간 단서"를 찾는 데 특화되어 있다면, RNN은 텍스트, 음성, 주가 데이터 등 **"시간의 흐름(순서)"**이 중요한 데이터를 다루는 전문가입니다.

### ⏳ 1. CNN과의 결정적 차이: 기억력(Hidden State)
* **CNN**: 1초 전의 일을 기억하지 못합니다. 데이터를 섞어서 입력해도 특징만 찾는다면 결과는 같습니다.
* **RNN**: **메모장(은닉 상태, Hidden State)**을 가집니다. 단어를 순서대로 하나씩 읽으며 앞 단어의 "문맥(Context)"을 메모장에 계속 누적합니다. 때문에 어순이 다르면 완전히 다른 결과가 나옵니다.

### 🔠 2. 자연어 처리의 핵심: 임베딩 (Embedding)
컴퓨터는 "사과", "맛있다"라는 글자를 이해하지 못합니다. 단어장(Vocab)을 만들어 1번, 2번 등 고유 숫자를 부여한 뒤, 이를 연산 가능한 **밀집 벡터(Dense Vector)**로 바꿔주는 과정이 필수입니다.
```python
# 단어 번호를 8개의 소수점(벡터)의 조합으로 바꿔주는 마법의 층
self.embedding = nn.Embedding(num_embeddings=vocab_size, embedding_dim=8)
```

### 🧠 3. RNN의 작동 흐름 (감성 분석 예시)
문장: "이 영화 정말 최고" (단어 4개)
